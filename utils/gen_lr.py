...
# def lower_resolution_images(input_dir, output_dir, log_label):
def lower_resolution_images(input_dir, output_dir, count, bar): 
    items = os.listdir(input_dir)
    for item in items:
        if not item.endswith('.png'):
            continue

        # path to input files and output files
        in_file = os.path.join(input_dir, item)
        out_file = os.path.join(output_dir, item) 

        # reading images and transpose
        img = cv2.imread(in_file)
        img = img * 1.0 / 255
        img = torch.from_numpy(np.transpose(img[:, :, [2, 1, 0]], (2, 0, 1))).float()

        # images resize and record the consuming time
        scale = 1 / 4
        total_time = 0
        for i in range(10):
        ...
        count += 100 / len(items)
        bar.setValue(count)
        # log_label.setText('--- write to file : {} ---\n'.format(out_file.split('\\')[-1]))
